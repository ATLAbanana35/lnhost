...
<body>
    <div class="bento">
        <header class="bento_child">
            <h1>LNHost Server Apps Settings</h1>
        </header>
    ...
    function runNextcloudSetup(dbPassword) {
        ...
        apiKeyR.onload = function() {
            ...
            xhr.open(
                "GET",
                "http://localhost:8080/run_nextcloud_setup?db_password=" +
                encodeURIComponent(dbPassword) +
                "&tmp_api_key=" +
                encodeURIComponent(apiKeyR.responseText.trim())
            );
    ...
    function runKChatcloudSetup(dbPassword) {
        ...
        apiKeyR.onload = function() {
            ...
            xhr.open(
                "GET",
                "http://localhost:8080/run_kchat_setup?db_password=" +
                encodeURIComponent(dbPassword) +
                "&tmp_api_key=" +
                encodeURIComponent(apiKeyR.responseText.trim())
            );
    ...
    function runWordPressSetup(dbPassword) {
        ...
        apiKeyR.onload = function() {
            ...
            xhr.open(
                "GET",
                "http://localhost:8080/run_wordpress_setup?db_password=" +
                encodeURIComponent(dbPassword) +
                "&tmp_api_key=" +
                encodeURIComponent(apiKeyR.responseText.trim())
            );
    ...
    function runSerendipitySetup(dbPassword) {
        ...
        apiKeyR.onload = function() {
            ...
            xhr.open(
                "GET",
                "http://localhost:8080/run_serendipity_setup?db_password=" +
                encodeURIComponent(dbPassword) +
                "&tmp_api_key=" +
                encodeURIComponent(apiKeyR.responseText.trim())
            );
        ...
        XML2.onload = function name(params) {
            ...
            apps.forEach((app) => {
                ...
                document
                    ...
                    .addEventListener("click", () => {
                        Swal.fire({
                            title: "Are you sure you want to delete your ENTIRE APP?",
                            ...
                            if (result.isConfirmed) {
                                Swal.fire({
                                    title: "Sure, all your data will be deleted AND your bdd! [i] You will see the bdd remove script on live",
                                    text: "ALL YOUR DATA, NO COMEBACK, (you have got 2Gb of FREE memory, so you can install another app, all apps can fit in ~1.7 Gb)",
                                    icon: "warning",
                                    showCancelButton: true,
                                    confirmButtonColor: "#d33",
                                    cancelButtonColor: "#3085d6",
                                    ...
                                    if (result.isConfirmed) {
                                        Swal.fire({
                                            ...
                                            Swal.fire({
                                                title: "Uninstalling Serendipity...",
                                                html: '<div id="logContainer"></div>', // Conteneur pour les logs
                                                showCloseButton: false,
                                                showConfirmButton: false,
                                                allowOutsideClick: false, // Empêche la fermeture en cliquant à l'extérieur de l'alerte
